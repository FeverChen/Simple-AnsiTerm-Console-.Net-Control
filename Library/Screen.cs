...
namespace Library.VT100
{
    public class Screen : IAnsiDecoderClient, IEnumerable<Screen.Character>
    {
        ...
        private int m_slidingBaseRow = 0;
        private int m_maxRow = 0;
        private Point m_bottomCursorPosition = new Point(0,0);

        public Point BottomCursorPosition
        {
            get { return m_bottomCursorPosition; }
            set { m_bottomCursorPosition = value; }
        }
        ...
        public Point CursorPosition
        {
            ...
            set
            {
                ...
                {
                    int pos = value.Y * Height + value.X;
                    int pos2 = BottomCursorPosition.Y * Height + BottomCursorPosition.X;
                    if (pos > pos2)
                    {
                        BottomCursorPosition = value;
                    }
                    if (value.Y > m_maxRow)
                    {
                        int shiftRows = value.Y - m_maxRow;
                        m_maxRow = value.Y;
                        m_slidingBaseRow = (value.Y + 1) % Height;
        ...
        public Character this[int _column, int _row]
        {
            get
            {
                ...
                if (_row > Height - 1)
                {
                    _row = (Height - 1) - ((m_maxRow - _row) % Height);
                }
                return m_screen[_column, (m_slidingBaseRow + _row) % Height];
            }
            set
            {
                //CheckColumnRow(_column, _row);
                ...
                m_screen[_column, (m_slidingBaseRow + _row) % Height] = value;
            }
        }
